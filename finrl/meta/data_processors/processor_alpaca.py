...
class AlpacaProcessor:
    ...
    def download_data(
        ...
        day_delta = 86400000000000  # pd.Timedelta('1D') in nanoseconds
        if pd.Timedelta(time_interval).total_seconds() * 1e9 < day_delta:
    ...
    def clean_data(self, df):
        ...
        trading_days = self.get_trading_days(start=pd.to_datetime(self.start).tz_localize('UTC'), end=pd.to_datetime(self.end).tz_localize('UTC')    )
